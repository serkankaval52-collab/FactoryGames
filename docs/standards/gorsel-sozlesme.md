...
maliyetini değiştirir. (1) Kaynak disiplini (R3 lisans, T1 transform) ayrı ve
üstte kalır. (2) Vekiller yanılabilir; insan sigortası Aşama 8'dir (G5 beyanı).

**Sayı disiplini:** eşikler Ek C'dedir (Sözleşme-8); burada yapı + yöntem.
Varsayılanlar dış standart/politika sabiti; kalibrasyon şerhli olan işaretli.
...
- `StreamingAssets/palette.json` (tek kaynak; Aşama 2'de yazılır, sonra
  ...
  ana özne, vurgu, tehlike, UI metin, UI zemin. Kaynak renk, kartın stil ailesi
...
- Ölçü: pikseller rampaya karşı; rampa dışı piksel oranı ≤ Ek C
  ...
  HARİÇ — kenar yumuşatma G2'nin konusu; yoksa her kenar palet ihlali (A1.1).

## G2 Sprite geometrisi (CI)

- Tüm sprite'lar tek `pixels-per-unit`; ana özneler manifest çözünürlük
  bandında; kenar yarı-saydam oranı ≤ Ek C `gorsel_alpha_sacak_yuzde`;
...
- Okunabilirlik (mutlak, her özne VE ikon): doluluk oranı Ek C
  `gorsel_siluet_doluluk_band` içinde + dış çeper karmaşıklığı ≥ Ek C
  `gorsel_siluet_ceper_min` + 32px'te tek parça. İkon çift testine GİRMEZ —
  mutlak testten geçer (A1.5).
...
- Şerh (A1.4): oran metin okunabilirliği içindir; özne ayrımında kaba vekildir
  (zıt tonlar aynı parlaklıkta düşük oran verir). Asıl güvence G6'dadır.

## G5 Tutarlılık üçlüsü (CI + Aşama 8 insan)

- Manifest zorunlu alanları: perspektif, çizgi kalınlığı bandı, gölge yönü
  (8 yön/"yok"); lint oyun içinde tek üçlü ister — dağınıklığın mekanik bekçisi.
...
- palette.json taban rol renkleri üç simülasyondan geçirilir (protanopi,
  döteranopi, tritanopi — lint içi sabit matrisler); kritik çiftler (tehlike↔
  vurgu, tehlike↔arka plan, ana özne↔arka plan) her simülasyonda da Ek C
  `gorsel_kontrast_ana_ozne` üstünde kalır (A1.3 — global hedefin karşılığı).
...
- Bütünleşik yükseklik Ek C `ses_lufs_band`; tepe ≤ Ek C `ses_tepe_dbtp`; tekil
  SFX ≤ Ek C `ses_sfx_sure_tavan_sn`; format/örnekleme manifestte tek değer.
  Ölçü: FFmpeg ebur128 — kurulum satır 10 (A1.2). B6 sessizde-oynama kapsamı kalır.
...
**İlk-koşu notu:** kapılar ilk koşuda da çalışır; "veri yok" diye sessizce
atlanmaz — şerhli eşikler varsayılanla uygulanır, ölçüm rapora düşer.
