...
| İnsan taahhüt alanları (A6.1/A6.4) | ÇALIŞTI | `insan_yanit_tavan_rubrik_gun`, `insan_yanit_tavan_t3_gun`, `insan_yanit_tavan_magaza_form_gun`, `insan_yanit_tavan_karar_gun`, `insan_yanit_tavan_sanat_gun` kullanıcının 0A-6 taahhüdünden gelir (ölçüm değil); aşım kapıyı DURDURMAZ — A10 hat bakımına satır düşer. `halka_davet_tavan_gun` dolarsa A5 eldeki N ile devam eder (N rapora, şerhli) |
