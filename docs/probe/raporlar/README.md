...
- Push öncesi zorunlu kontroller — herhangi biri kırmızıysa DUR, mimara bildir:
  ...
  2. `py -3.12 tools/arac_suzgec.py` → exit 0
  3. `py -3.12 tools/esik_kapsama.py` → exit 0
- Yerel git kimliği gerekirse REPO-İÇİ (global YOK): `git config user.name "FactoryGames
  Executor"` + `git config user.email "serkankaval52-collab@users.noreply.github.com"`
  (public tarihe gerçek e-posta GİRMEZ).
